...
const INTENT_COLORS: Record<Intent, string> = {
  ...
  summarize: "text-foreground/70 bg-foreground/[0.08] border-foreground/20",
...
function PolarityBar({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-20 text-[10px] text-foreground/30">{label}</span>
      ...
      <span className="w-8 text-right text-[10px] text-foreground/40">
        {percent}%
      </span>
    </div>
  );
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mb-6 flex items-center gap-3 rounded-xl border border-foreground/[0.08] bg-card/70 px-4 py-3 backdrop-blur-xl">
        ...
        <input
          ...
          className="w-full bg-transparent text-sm text-foreground placeholder:text-foreground/20 outline-none"
        />
        {search.trim() && (
        ...
        <div className="space-y-3">
          {contextResults.map((result) => {
            ...
            return (
              <div
                key={result.id}
                className={`rounded-2xl border bg-card/70 p-5 backdrop-blur-xl transition-all ${
                  isExpanded
                    ? "border-neon-cyan/35 bg-card/80"
                    : "border-foreground/[0.08] hover:border-foreground/[0.14] hover:bg-card/80"
                }`}
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <p className="flex-1 text-sm font-medium text-foreground/90">
                ...
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-md border border-foreground/10 bg-foreground/[0.04] px-2 py-0.5 text-[10px] text-foreground/50">
                    <Gauge className="h-3 w-3" />
                    {Math.round(result.intent.confidence * 100)}% confianca
                  </span>
                  <span className="rounded-md border border-foreground/10 bg-foreground/[0.04] px-2 py-0.5 text-[10px] text-foreground/50">
                ...
                <div className="flex flex-wrap items-end justify-between gap-4">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <Tags className="h-3 w-3 text-foreground/25" />
                    {result.context.domains.map((domain) => (
                      <span
                        key={`${result.id}-${domain.domain}`}
                        className="rounded-full bg-foreground/[0.06] px-2 py-0.5 text-[10px] text-foreground/40"
                      >
                        {domain.domain}
                      </span>
                    ))}
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-foreground/[0.06] pt-3">
                ...
                {isExpanded && (
                  <div className="mt-4 rounded-xl border border-foreground/[0.08] bg-black/30 p-3">
                    <div className="mb-2">
                    ...
                    <div className="mb-3 max-h-56 space-y-2 overflow-y-auto rounded-lg border border-foreground/[0.06] bg-black/25 p-2.5">
                            ...
                            >
                              <div
                                className={`max-w-[92%] rounded-xl border px-2.5 py-2 ${
                                  isUser
                                    ? "border-neon-cyan/35 bg-neon-cyan/10 text-foreground"
                                    : "border-foreground/[0.08] bg-foreground/[0.03] text-foreground/80"
                                }`}
                              >
                    ...
                    <div className="rounded-lg border border-foreground/[0.08] bg-black/30 p-2">
                      <textarea
                        ...
                        className="h-20 w-full resize-none bg-transparent text-xs text-foreground placeholder:text-foreground/30 outline-none"
                      />
