...
export function WorkspaceCard({
  ...
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-white/[0.08] bg-neutral-900/70 backdrop-blur-xl transition-all duration-300",
        colors.borderHover,
        colors.glow
      )}
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[10px] text-white/30">
            ...
            {workspace.status === "archived" && (
              <span className="ml-1 rounded-full border border-white/[0.08] bg-white/[0.04] px-1.5 py-0.5 text-[9px] uppercase tracking-wide text-white/30">
                Arquivado
              </span>
            )}
          </div>
          <Link
            href={`/dashboard/workspaces/${workspace.id}`}
            className="inline-flex items-center gap-0.5 text-[10px] text-white/25 opacity-0 transition-all group-hover:text-white/50 group-hover:opacity-100"
          >
            Abrir
