      ...
      <DialogContent className="border-foreground/[0.08] backdrop-blur-xl sm:max-w-md">
        ...
        {item && (
          <div className="dialog-section mb-3">
            <p className="truncate text-sm font-medium text-foreground/70">{item.title}</p>
            <p className="text-xs text-foreground/30">{item.source} · {item.author}</p>
          </div>
        )}
        ...
        <div className="max-h-64 space-y-0.5 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-8 text-center">
              <Users2 className="h-8 w-8 text-foreground/10" />
            ...
            filtered.map((conn) => (
              <button
                ...
                className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors hover:bg-foreground/[0.05]"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground/80">{conn.name}</p>
                  <p className="truncate text-xs text-foreground/30">{conn.role}</p>
                </div>
                <Share2 className="h-3.5 w-3.5 shrink-0 text-foreground/15" />
              </button>
            ))
          )}
        </div>
